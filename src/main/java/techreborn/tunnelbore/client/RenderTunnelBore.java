...
public class RenderTunnelBore extends TileEntitySpecialRenderer<TileTunnelboreController> {
	...
	public void render(TileTunnelboreController te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		renderBlock(Blocks.IRON_BLOCK.getDefaultState(), te.getPos(), partialTicks, te);
		if(te.isMoving()){
			for(BlockData blockData : te.structure.getMovingBlocks()){
				renderBlock(blockData.getBlockState(), blockData.getOldPos(), partialTicks, te);
	...
	private void renderBlock(IBlockState state, BlockPos anchor, float partialTicks, TileTunnelboreController tileTunnelboreController) {
